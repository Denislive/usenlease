...
class Image(models.Model):
    ...
    def clean(self):
        # Limit to a maximum of 4 images per equipment
        if self.pk is not None:  # Only check for existing instances
            if self.equipment.images.count() >= 4:
                raise ValidationError("You can only upload a maximum of 4 images for this equipment.")
